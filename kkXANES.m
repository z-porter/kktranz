function [ref, fitxanes, varargout] = kkXANES(xanes, E_given, ElementName,...
    E_ROI_bounds, ifplot)
%function for calculating f' and f'' from an input normalized XANES mu
%I call (f'=real[f]-f_0) ref and I call (f''=imag[f]) imf
%
%Energy units are in eV
%
%INPUTS
%xanes, an array of XANES data, processed by sixpack or similar software 
%       MUST remove background AND normalize to 1 PRIOR to this function
%
...
%
%fitxanes, the f'' values for the input energy range E_given
%       calculated by shifting the first value and stretching the final
%       value to splice into existing f'' table
%
...
xanes = interp1(E_given,xanes,E_corrected,interpmethod);
...
%% load experimental XANES data
[~,EidxLow,~]  = find(E >= E_given(1)  );
[~,EidxHigh,~] = find(E <= E_given(end));
EidxXANES = intersect(EidxLow,EidxHigh);
...
imf = interp1(fpp(1,:).*1e3, -1.*fpp(2,:), E);
...
%% fit XANES to table
shiftY = (imf(EidxXANES(1))-xanes(1));
stretch= (imf(EidxXANES(end))-shiftY)/xanes(end);
fitxanes = (xanes.*stretch)+shiftY;
imf_full = [imf(1:EidxXANES(1)-1), fitxanes,...
            imf(EidxXANES(end)+1:end) ];


%% KK transform into f'
ref_ROI = kkrebook2z(E, imf_full, 0, E_ROI(1), E_ROI(end));
ref = interp1(E, ref_ROI, E_given, interpmethod);
ref_ROI = interp1(E, ref_ROI, E_ROI, interpmethod);
...
if (nargin>3 && ifplot)
    ...
    plot(E_given, fitxanes, 'g', 'LineWidth',2);
    title(['XANES, ', ElementName, ' Edge']);
    xlabel('Energy (eV)'); ylabel('f"');
    legend({'table', 'xanes'}, 'Location', 'southeast');
    ...
    xlabel('Energy (eV)'); ylabel('f''');
    legend({'table', 'KK of xanes'}, 'Location', 'southeast');
end
